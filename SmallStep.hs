...
-- Semântica Operacional Pequeno Passo para Expressões Aritméticas, Booleanas e Comandos
...
smallStepC (Swap (Var x) (Var y), s) = let vx = procuraVar s x
                                      in let vy = procuraVar s y
                                      in (Skip, mudaVar (mudaVar s x vy) y vx)

smallStepC (DAtrrib (Var x1) (Var x2) (Num n1) (Num n2), s) =
  let s' = mudaVar s x1 n1
      s'' = mudaVar s' x2 n2 
  in (Skip, s'')

smallStepC (DAtrrib (Var x1) (Var x2) (Num n1) e2, s) =
  let (e2', s1) = smallStepE (e2, s)
  in (DAtrrib (Var x1) (Var x2) (Num n1) e2', s1)

smallStepC (DAtrrib (Var x1) (Var x2) e1 e2, s) =
  let (e1', s1) = smallStepE (e1, s)
      (e2', s2) = smallStepE (e2, s) -- Mantemos a memória original para e2
  in (DAtrrib (Var x1) (Var x2) e1' e2', s2)
